...
public class PowerWidget extends AppWidgetProvider {
	...
	public static void updateWidgetDone(Context context) {
		...
		views.setInt(R.id.power_button, "setImageResource", R.drawable.power_off);
	...
	public static void updateWidget(Context context, DataCollector p) {
		AppWidgetManager manager = AppWidgetManager.getInstance(context);
		ComponentName comp = new ComponentName(context, PowerWidget.class);
		...
		for (int id : manager.getAppWidgetIds(comp)) {
			...
			views.setOnClickPendingIntent(R.id.power_button, pendingIntent);
			views.setInt(R.id.power_button, "setImageResource", R.drawable.power_on);
			...
			try {
				...
				if (val != null) {
					ObjectInputStream objin = new ObjectInputStream(new ByteArrayInputStream(HexEncode.decode(val)));
					for (int i = 0; i < text_ids.length; i++) {
