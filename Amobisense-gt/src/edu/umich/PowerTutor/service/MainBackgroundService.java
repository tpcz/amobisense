...
//android.os.Debug.startMethodTracing("pt.trace");
  ...
  //android.os.Debug.stopMethodTracing();
    ...
    // Done in main loop finalizer
    //if (DataCollector.getInstance() != null) {
    //	DataCollector.getInstance().clearComponentResources();
    //}
  ...
  public void showNotification(){
    ...
    CharSequence contentTitle = getApplicationContext().getString(R.string.app_name);  		 // expanded message title
  ...
  private final ICounterService.Stub binder = 
    new ICounterService.Stub() {
      public String[] getComponents() {
        return dataCollector.getComponents();
      }

      public int[] getComponentsMaxPower() {
        return dataCollector.getComponentsMaxPower();
      }

      public int getNoUidMask() {
        return dataCollector.getNoUidMask();
      }

      public int[] getComponentHistory(int count, int componentId, int uid) {
        return dataCollector.getComponentHistory(count, componentId, uid, -1);
      }

      public long[] getTotals(int uid, int windowType) {
        return dataCollector.getTotals(uid, windowType);
      }

      public long getRuntime(int uid, int windowType) {
        return dataCollector.getRuntime(uid, windowType);
      }

      public long[] getMeans(int uid, int windowType) {
        return dataCollector.getMeans(uid, windowType);
      }

      public byte[] getUidInfo(int windowType, int ignoreMask) {
        UidInfo[] infos = dataCollector.getUidInfo(windowType, ignoreMask);
      ...
      public long getUidExtra(String name, int uid) {
        return dataCollector.getUidExtra(name, uid);
      }
    };
      ...
      if(intent.getAction().equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
        ...
        try {
          if ((Boolean)extra.get("state")) {
            dataCollector.writeToPowerLog("airplane-mode on\n");
          } else {
            dataCollector.writeToPowerLog("airplane-mode off\n");
          }  
        ...
        dataCollector.writeToPowerLog("battery low\n");
      } else if(intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED)) {
        dataCollector.writeToPowerLog("battery-change " +
    ...
    public void onServiceStateChanged(ServiceState serviceState) {
      switch(serviceState.getState()) {
        case ServiceState.STATE_EMERGENCY_ONLY:
          dataCollector.writeToPowerLog("phone-service emergency-only\n");
          break;
        case ServiceState.STATE_IN_SERVICE:
          dataCollector.writeToPowerLog("phone-service in-service\n");
          switch(phoneManager.getNetworkType()) {
            case(TelephonyManager.NETWORK_TYPE_EDGE):
              dataCollector.writeToPowerLog("phone-network edge\n");
              break;
            case(TelephonyManager.NETWORK_TYPE_GPRS):
              dataCollector.writeToPowerLog("phone-network GPRS\n");
              break;
            case 8:
              dataCollector.writeToPowerLog("phone-network HSDPA\n");
              break;
            case(TelephonyManager.NETWORK_TYPE_UMTS):
              dataCollector.writeToPowerLog("phone-network UMTS\n");
              break;
            default:
              dataCollector.writeToPowerLog("phone-network " +
                                        phoneManager.getNetworkType() + "\n");
          }
          break;
        case ServiceState.STATE_OUT_OF_SERVICE:
          dataCollector.writeToPowerLog("phone-service out-of-service\n");
          break;
        case ServiceState.STATE_POWER_OFF:
          dataCollector.writeToPowerLog("phone-service power-off\n");
          break;
      }
    }
    ...
    public void onCallStateChanged(int state, String incomingNumber) {
      switch(state) {
        case TelephonyManager.CALL_STATE_IDLE:
          dataCollector.writeToPowerLog("phone-call idle\n");
          break;
        case TelephonyManager.CALL_STATE_OFFHOOK:
          dataCollector.writeToPowerLog("phone-call off-hook\n");
          break;
        case TelephonyManager.CALL_STATE_RINGING:
          dataCollector.writeToPowerLog("phone-call ringing\n");
          break;
      }
    }
    ...
    public void onDataConnectionStateChanged(int state) {
      switch(state) {
        case TelephonyManager.DATA_DISCONNECTED:
          dataCollector.writeToPowerLog("data disconnected\n");
          break;
        case TelephonyManager.DATA_CONNECTING:
          dataCollector.writeToPowerLog("data connecting\n");
          break;
        case TelephonyManager.DATA_CONNECTED:
          dataCollector.writeToPowerLog("data connected\n");
          break;
        case TelephonyManager.DATA_SUSPENDED:
          dataCollector.writeToPowerLog("data suspended\n");  
          break;
      }
    }

    public void onSignalStrengthChanged(int asu) {
      dataCollector.writeToPowerLog("signal " + asu + "\n");
    }
  };
}
